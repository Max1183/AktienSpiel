...
function StockHoldings() {
    ...
    useEffect(() => {
        ...
        const chartData = getData("team").portfolio_history;
                    ...
                    <div className="row row-cols-sm-2 row-cols-md-3 g-2 m-0">
                        <InfoField
                            ...
                            value={formatCurrency(
                                team.portfolio_value - team.balance
                            )}
                        />
                        <InfoField
                            label="Barbestand"
                            value={formatCurrency(team.balance)}
                        />
                        <InfoField
                            label="Gesamter Depotwert"
                            value={formatCurrency(team.portfolio_value)}
                        />
                        <InfoField
                            label="Performance abs."
                            value={formatCurrency(
                                team.portfolio_value - 100000
                            )}
                        />
                        <InfoField
                            label="Performance %"
                            value={
                                (
                                    (team.portfolio_value / 100000) * 100 -
                                    100
                                ).toFixed(2) + "%"
                            }
                        />
                    ...
                    stockholdings.length > 0 ? (
                        <div className="list-group rounded">
                            {stockholdings.map((stockHolding) => (
                                <StockDetailLink
                                    stock_id={stockHolding.stock.id}
                                >
                                    <div className="d-flex w-100 justify-content-between">
                                        <h5 className="mb-1">
                                            {stockHolding.stock.name}
                                        </h5>
                                        <small>
                                            Kurs:{" "}
                                            {formatCurrency(
                                                stockHolding.stock.current_price
                                            )}
                                        </small>
                                    </div>
                                    <p className="mb-1">
                                        Anzahl: {stockHolding.amount}
                                    </p>
                                    <small>
                                        Gesamtwert:{" "}
                                        {formatCurrency(
